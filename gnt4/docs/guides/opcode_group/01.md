...
- Comparison Register (`cr`) = reg_p[0x15]
  - Values are set to this in opcode group 04, 05, 06, and 07.
- Return Addresses (`ra`) = reg_p[0x17]
  - The stack of return addresses for subroutine calls
...
- Parameters: 0 or 1 (Based on seq_opcode_parse_opts)
- Terminate: Always
...
- Parameters: 1 or 2 (Based on seq_opcode_parse_opts)
...
Push the return address to the `ra` stack abd unconditional branch to the offset stored in `param1`.
...
Push the return address to the `ra` stack and branch to the offset stored in `param1` if `cr == 0`.
...
Pop the return address from `ra` and unconditional branch to it.
...
Pop the return address from `ra` and branch to it if it is `== 0`.
...
Set `pc` and `ctr` to values from `seq_opcode_parse_opts`. Push `pc + 4` to the `ra` stack
...
- Parameters: Variable based on seq_opcode_parse_opts and parameter
