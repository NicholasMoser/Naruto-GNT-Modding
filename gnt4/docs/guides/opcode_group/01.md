...
- Parameters: 0 or 1 (Based on `seq_get_op`)
- Terminate: Always
...
- Parameters: 1 or 2 (Based on `seq_get_op`)
...
Set `pc` and `ctr` to values from `seq_get_op`. Push `pc + 4` to the `ra` stack
...
- Parameters: Variable based on `seq_get_op` and parameter
