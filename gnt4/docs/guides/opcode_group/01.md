...
- Parameters: 0 or 1 (Based on `get_effective_address`)
- Terminate: Always
...
- Parameters: 1 or 2 (Based on `get_effective_address`)
...
Set `pc` and `ctr` to values from `get_effective_address`. Push `pc + 4` to `sp`
...
Call `get_effective_address` and return `returnVal`. If `returnVal` is less than 0 OR `pc` is less than or equal to `returnVal` OR the value at `pc + 1` with offset `returnVal` is equal to zero: add 1 to `pc` and then add the value at the new `pc`. Otherwise, set `pc` to `seq_p->seq_p_sp[0x17]` + `returnVal`.

There is an array of seq offsets after this opcode.

- Parameters: Variable based on `get_effective_address` and parameter
