...
Set `pc` and `ctr` to values from `seq_get_op`. Push `pc + 4` to `sp`
...
Call `seq_get_op` and return `returnVal`. If `returnVal` is less than 0 OR `pc` is less than or equal to `returnVal` OR the value at `pc + 1` with offset `returnVal` is equal to zero: add 1 to `pc` and then add the value at the new `pc`. Otherwise, set `pc` to `seq_p->seq_p_sp[0x17]` + `returnVal`.
