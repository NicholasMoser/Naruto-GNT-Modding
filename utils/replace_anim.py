...
subprocess.run([QUICK_BMS, "-Y", MOT_BMS, MOT_0000, FILES_0000])
subprocess.run([QUICK_BMS, "-Y", MOT_BMS, MOT_0001, FILES_0001])

# List animations in the .gnta files for 0000.mot
original_file = os.path.join(FILES_0001, '0.gnta')
original_size = os.path.getsize(original_file)
mot_files = os.listdir(FILES_0000)
for mot_file in mot_files:
    if os.path.getsize(os.path.join(FILES_0000, mot_file)) <= original_size:
        # QuickBMS only cannot replace files in archives with larger files than the original
        print(mot_file.split('.')[0])

# User input animation will replace 0001.mot from 0000.mot
animation = input('Please select from the above animations: ')
...
subprocess.run([QUICK_BMS, "-r", "-w", "-Y", "naruto_mot.bms", "0001.mot", "0001"])
