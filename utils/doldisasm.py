...
blacklistedInsns = {
    ...
    # TODO: find what these are supposed to be
    PPC_INS_MFICCR, PPC_INS_MFESR, PPC_INS_MFDEAR, PPC_INS_MTICCR,
    PPC_INS_MTDEAR, PPC_INS_MTESR, PPC_INS_MFASR
}
...
def get_label_callback(address, offset, insn, bytes):
    ...
    '''
    # Detect split load (low part)
    if insn.id in {PPC_INS_ADDI, PPC_INS_ORI} and insn.operands[1].reg in lisInsns:
        prevInsn = lisInsns[insn.operands[1].reg]
        if prevInsn != None:
            # Record the combined value into the splitDataLoads dict
            value = prevInsn.operands[1].imm << 16
            value |= insn.operands[2].imm
            #print("\tsplit value: 0x%08X" % value)
            splitDataLoads[prevInsn.address] = value
            splitDataLoads[insn.address] = value
            if is_label_candidate(value):
                labels.add(value)
            # Attempt to detect initialization of r2 and r13
            # this is marked by "lis r2, XXXXXXXX@h" "ori r2, r2, XXXXXXXX@l"
            if insn.id == PPC_INS_ORI and insn.operands[0].reg == insn.operands[1].reg:
                if r2_addr == None and insn.operands[0].reg == PPC_REG_R2:
                    r2_addr = value
                    print('# DEBUG: set r2 to 0x%08X' % value)
    ...
    # Detect split load (high part)
    if insn.id == PPC_INS_LIS:
        #print("\tlis %i 0x%08X" % (insn.operands[0].reg, insn.operands[1].imm))
        lisInsns[insn.operands[0].reg] = insn
    # Remove lis data if register is overwritten
    elif (not insn.id in {PPC_INS_STW}) and len(insn.operands) >= 1 and insn.operands[0].type == PPC_OP_REG:
        #print("\tremove %i" % insn.operands[0].reg)
        lisInsns[insn.operands[0].reg] = None
    '''
...
# Dumps the instruction as raw bytes instead of disassembling
def insn_to_text_raw_bytes(insn):
    return ".byte 0x%02X, 0x%02X, 0x%02X, 0x%02X  /* %s %s */" % (insn.bytes[0], insn.bytes[1], insn.bytes[2], insn.bytes[3], insn.mnemonic, insn.op_str)
...
def insn_to_text(insn):
    ...
    if insn.id == PPC_INS_BDNZ and (insn.bytes[0] & 1):
        return insn_to_text_raw_bytes(insn)
    ...
    '''
    # Split loads (high part)
    elif insn.id == PPC_INS_LIS and insn.address in splitDataLoads:
        return "%s %s, %s@h" % (insn.mnemonic, insn.reg_name(insn.operands[0].value.reg), addr_to_label(splitDataLoads[insn.address]))
    # Split loads (low part)
    elif insn.id in {PPC_INS_ADDI, PPC_INS_ORI} and insn.address in splitDataLoads:
        return "%s %s, %s, %s@l" % (insn.mnemonic, insn.reg_name(insn.operands[0].value.reg), insn.reg_name(insn.operands[1].value.reg), addr_to_label(splitDataLoads[insn.address]))
    '''
    # Handle split loads (high part)
    if insn.id == PPC_INS_LIS and insn.address in splitDataLoads:
    ...
    # Sign-extend immediate values because GNU assembler is too dumb to handle them
    if insn.id in {PPC_INS_ADDI, PPC_INS_ADDIC, PPC_INS_SUBFIC, PPC_INS_MULLI} and (insn.operands[2].imm & 0x8000):
        return "%s %s, %s, %i" % (insn.mnemonic, insn.reg_name(insn.operands[0].value.reg), insn.reg_name(insn.operands[1].value.reg), insn.operands[2].imm - 0x10000)
    elif (insn.id == PPC_INS_LI or insn.id == PPC_INS_CMPWI) and (insn.operands[1].imm & 0x8000):
        return "%s %s, %i" % (insn.mnemonic, insn.reg_name(insn.operands[0].value.reg), insn.operands[1].imm - 0x10000)
    # cntlz -> cntlzw
    elif insn.id == PPC_INS_CNTLZW:
        return "cntlzw %s" % insn.op_str
    # Dunno why GNU assembler doesn't accept this
    elif insn.id == PPC_INS_LMW and insn.operands[0].value.reg == PPC_REG_R0:
        return insn_to_text_raw_bytes(insn)
...
def disasm_ps(inst):
    ...
    if opcode == 6:  # doesn't seem to be used
        ...
        return '%s p%i, (r%i + r%i), %i, qr%i' % (mnemonic, FD, RA, RB, WX, IX)
    if opcode == 7:
        mnemonic = 'psq_stux' if inst & 0x40 else 'psq_stx'
        return '%s p%i, r%i, r%i, %i, qr%i' % (mnemonic, FS, RA, RB, WX, IX)
    if opcode == 18:
        return 'ps_div p%i, p%i, p%i' % (FD, FA, FB)
    if opcode == 20:
        return 'ps_sub p%i, p%i, p%i' % (FD, FA, FB)
    if opcode == 21:
        return 'ps_add p%i, p%i, p%i' % (FD, FA, FB)
    if opcode == 23:
        return 'ps_sel p%i, p%i, p%i' % (FD, FA, FC)
    if opcode == 24:
        return 'ps_res p%i, p%i' % (FD, FB)
    if opcode == 25:
        return 'ps_mul p%i, p%i, p%i' % (FD, FA, FC)
    if opcode == 26:
        return 'ps_rsqrte p%i, p%i' % (FD, FB)
    if opcode == 28:
        return 'ps_msub p%i, p%i, p%i, p%i' % (FD, FA, FC, FB)
    if opcode == 29:
        return 'ps_madd p%i, p%i, p%i, p%i' % (FD, FA, FC, FB)
    if opcode == 30:
        return 'ps_nmsub p%i, p%i, p%i, p%i' % (FD, FA, FC, FB)
    if opcode == 31:
        return 'ps_nmadd p%i, p%i, p%i, p%i' % (FD, FA, FC, FB)
    if opcode == 10:
        return 'ps_sum0 p%i, p%i, p%i, p%i' % (FD, FA, FC, FB)
    if opcode == 11:
        return 'ps_sum1 p%i, p%i, p%i, p%i' % (FD, FA, FC, FB)
    if opcode == 12:
        return 'ps_muls0 p%i, p%i, p%i' % (FD, FA, FC)
    if opcode == 13:
        return 'ps_muls1 p%i, p%i, p%i' % (FD, FA, FC)
    if opcode == 14:
        return 'ps_madds0 p%i, p%i, p%i, p%i' % (FD, FA, FC, FB)
    if opcode == 15:
        return 'ps_madds1 p%i, p%i, p%i, p%i' % (FD, FA, FC, FB)

    opcode = (inst >> 1) & 0x3FF
    if opcode == 40:
        return 'ps_neg p%i, p%i' % (FD, FB)
    if opcode == 72:
        return 'ps_mr p%i, p%i' % (FD, FB)
    if opcode == 136:
        return 'ps_nabs p%i, p%i' % (FD, FB)
    if opcode == 264:
        return 'ps_abs p%i, p%i' % (FD, FB)
    if opcode in {0, 32, 64, 96}:
        mnemonics = ['ps_cmpu0', 'ps_cmpo0', 'ps_cmpu1', 'ps_cmpo1']
        mnemonic = mnemonics[(inst >> 6) & 3]
        i = (inst & 0x03800000) >> 23
        return '%s cr%i, p%i, p%i' % (mnemonic, i, FA, FB)
    if opcode == 528:
        return 'ps_merge00 p%i, p%i, p%i' % (FD, FA, FB)
    if opcode == 560:
        return 'ps_merge01 p%i, p%i, p%i' % (FD, FA, FB)
    if opcode == 592:
        return 'ps_merge10 p%i, p%i, p%i' % (FD, FA, FB)
    if opcode == 624:
        return 'ps_merge11 p%i, p%i, p%i' % (FD, FA, FB)
    if opcode == 1014:
        if not (inst & 0x03e00000):
            if (inst & 1) == 0:
                return 'dcbz_l r%i, r%i' % ((inst & 0x001f0000) >> 16, (inst & 0x0000f800) >> 11)
    return ".4byte 0x%08X # unknown ps instruction" % inst
...
def disasm_ps_mem(inst, idx):
    ...
    if idx == 61:
        mnemonic = 'psq_stu'
    return '%s p%i, %i(r%i), %i, qr%i' % (mnemonic, RS, disp, RA, W, I)
...
def disassemble_callback(address, offset, insn, bytes):
    ...
    if insn == None:
        ...
        if idx == 4:
            print('%s\t%s' % (prefixComment, disasm_ps(raw)))
        elif idx in {56, 57, 60, 61}:
            print('%s\t%s' % (prefixComment, disasm_ps_mem(raw, idx)))
        else:
            print('%s\t.byte 0x%02X, 0x%02X, 0x%02X, 0x%02X  /* unknown instruction */' % (prefixComment, bytes[0], bytes[1], bytes[2], bytes[3]))
    else:
        print('%s\t%s' % (prefixComment, insn_to_text(insn)))
for i in range(0, 7):
    if textSizes[i] != 0:
        print(".section .text%i, \"ax\"  # 0x%08X - 0x%08X" % (i, textAddresses[i], textAddresses[i] + textSizes[i]))
...
for i in range(0, 11):
    ...
    if size == 0:
        continue
    print(".section .data%i, \"wa\"  # 0x%08X - 0x%08X" % (i, start, end))
...
address = bssAddress
print(".section .bss, \"wa\"  # 0x%08X - 0x%08X" % (start, end))
